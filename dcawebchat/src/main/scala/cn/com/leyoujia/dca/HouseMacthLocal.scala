...
object HouseMacthLocal {
  ...
  def process(spark: SparkSession): Unit = {
    ...
    val table1 = spark.table("tmp.client_intention_match_info") //客户需求信息
    ...
    val table1RDD = table1.rdd.map(x => {
      ...
      val purposeSchool = x.getAs[Long]("purpose_school").toInt
    ...
    var clientInfo = table1RDD.collect()


    //房客互动圈信息
    val table2RDD = table2.rdd.map(x => {
      val shareId = x.getAs[Long]("share_id").toInt
      val fh_id = x.getAs[Long]("fh_id").toInt
      ...
      val room = x.getAs[Long]("room").toInt
      val price = x.getAs[Double]("price")
      HouseInfo(shareId, fh_id, share_worker_id, city, area, place, room, price)
    })
    ...
    val tableDay = spark.table("tmp.house_match_client_day").rdd.map(d => d.getAs[String]("p_date")).first()
    var matchInfoCount: Map[String, Int] = Map()
    if (newDay != tableDay) {
      matchInfoCount = HashMap()
      //持久化新日期至hive表
      import spark.implicits._
      Seq(newDay).map(ClientMatchDay(_)).toDF.write.mode("overwrite").saveAsTable("tmp.house_match_client_day")
    } else {
      //得到客户匹配的次数，并转换成map
      matchInfoCount = spark.table("tmp.house_match_client_count").rdd.map(x => {
        val cid = x.getAs[String]("cid")
    ...
    for (house <- houseInfo) {
      ...
      for (client <- clientInfo) {
        ...
        if (house.price <= client.priceEnd && house.price >= client.priceStart && clentHouseTypeList.contains(house.room.toString) && clientPlace != 0 && house.place == clientPlace) {
          houseMachInfoArray += (MatchInfo(house.shareId, house.fh_id, house.share_worker_id,
            client.cid,
            client.objective,
            client.seeTime,
            client.area_code,
      ...
      if (houseMachInfoArray.map(_.cid).distinct.length < 3) {
        for (client <- clientInfo) {
          ...
          val clentHouseTypeList = client.houseType.split(",").toList
          if (house.price <= client.priceEnd && house.price >= client.priceStart && clentHouseTypeList.contains(house.room.toString) && clientArea != 0 && house.area == clientArea) {
            houseMachInfoArray += (MatchInfo(house.shareId, house.fh_id, house.share_worker_id,
              client.cid,
              client.seeTime,
              client.objective,
      ...
      if (recentlyMatchClient.count() > 0) {
        ...
        val sql = "SELECT shareId,fh_id, share_worker_id,insertWorkerId ,cid,objective,area_code,area_name,priceStart,priceEnd,houseType,houseAreaStart, houseAreaEnd,purposeSchool, purposeSchoolId,purposeSchoolName  FROM (SELECT shareId,fh_id,share_worker_id,cid,seeTime,objective,area_code,area_name,priceStart,priceEnd,houseType,houseAreaStart,houseAreaEnd, purposeSchool, purposeSchoolId,purposeSchoolName,updateTime,insertWorkerId,row_number() over(partition by fh_id,cid   ORDER BY  updateTime desc)rank   FROM match_info)s WHERE s.rank=1"
    ...
    matchInfo.toDF().show()
    matchInfo.toDF().repartition(1).write.mode("overwrite").saveAsTable("tmp.hsl_share_match_tmp")


    System.out.println("将客户匹配次数持久化到hive层")
    matchInfoCount.toArray.map(x => ClientMatchCount(x._1, x._2)).toSeq.toDF().show()
    matchInfoCount.toArray.map(x => ClientMatchCount(x._1, x._2)).toSeq.toDF().repartition(1).write.mode("overwrite").saveAsTable("tmp.house_match_client_count")
  }

  case class ClietInfo(cid: String, seeTime: String, objective: String, area_code: String, area_name: String, priceStart: Double, priceEnd: Double, houseType: String, houseAreaStart: Double, houseAreaEnd: Double, purposeSchool: Int, purposeSchoolId: String, purposeSchoolName: String, updateTime: String, insertWorkerId: String) extends Serializable

  case class HouseInfo(shareId: Int, fh_id: Int, share_worker_id: String, city: String, area: String, place: String, room: Int, price: Double) extends Serializable

  case class MatchInfo(shareId: Int, fh_id: Int, share_worker_id: String, cid: String, seeTime: String, objective: String, area_code: String, area_name: String, priceStart: Double, priceEnd: Double, houseType: String, houseAreaStart: Double, houseAreaEnd: Double, purposeSchool: Int, purposeSchoolId: String, purposeSchoolName: String, updateTime: String, insertWorkerId: String) extends Serializable

  case class MatchFinal(shareId: Int, fh_id: Int, share_worker_id: String, insertWorkerId: String, cid: String, objective: String, area_code: String, area_name: String, priceStart: Double, priceEnd: Double, houseType: String, houseAreaStart: Double, houseAreaEnd: Double, purposeSchool: Int, purposeSchoolId: String, purposeSchoolName: String) extends Serializable
